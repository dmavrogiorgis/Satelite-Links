...
for i=1:length(in_bits_xor)
    if i==1
        in_bits_xor(i)=xor(x1,in_bits(i));
    else
        in_bits_xor(i)=xor(in_bits(i-1),in_bits(i));
    end
end

in_symbols = 2*in_bits_xor-1; % [-1 1 -1 -1 1 -1 -1 -1 ].';Symbols we transmit {-1,+1}
...
for n=1:length(SNR)
    ...
    out_bits_xor = out_symbols>0;
    
    for i=1:length(out_bits_xor)
        if i==1
            out_bits(i)=xor(out_bits_xor(i),x1);
        else
            out_bits(i)=xor(out_bits_xor(i),out_bits(i-1));
        end
    end

    BER(n) = sum(in_bits~=out_bits)/N;
...
legend('Experimental with precoding BER', 'Theoretical BER');
grid on;
%print -depsc epsFig1